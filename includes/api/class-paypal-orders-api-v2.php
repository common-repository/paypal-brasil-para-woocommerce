...
class PayPal_Brasil_Orders_api_V2
{
	...
	/**
	 * Get the base URL for API webhook.
	 *
	 * @param null $mode
	 *
	 * @return string
	 */
	public function get_base_url_webhook($mode = null)
	{
		if (!$mode) {
			$mode = $this->mode;
		}
	...
	/**
	 * Get the base URL for API authentication.
	 *
	 * @param null $mode
	 *
	 * @return string
	 */
	public function get_base_url_authentication($mode = null)
	{
		if (!$mode) {
			$mode = $this->mode;
		}
	...
	public function get_base_url($mode = null)
	{
		...
		return ($mode === 'live') ? 'https://api.paypal.com/v2' : 'https://api.sandbox.paypal.com/v2';
	}
	...
	public function get_access_token($force = false)
	{
		...
		if ($code === 200) {
			set_transient(
				$this->access_token_transient_key,
		...
		throw new PayPal_Brasil_API_Exception(
			$code,
			__('Unable to get access token.', "paypal-brasil-para-woocommerce"),
			$response_body
		);
	}
	 ...
	 * @throws PayPal_Brasil_Connection_Exception
	 */
	public function get_payment($order_id, $headers = array(), $bn_code_key = null)
	{
	 ...
	 * @throws PayPal_Brasil_Connection_Exception
	 */
	public function execute_payment($order_id, $headers = array(), $bn_code_key = null)
	{
		...
		throw new PayPal_Brasil_API_Exception(
			$code,
			__('Unable to execute payment.', "paypal-brasil-para-woocommerce"),
			$response_body
		);
	}
	 ...
	 * @throws PayPal_Brasil_Connection_Exception
	 */
	public function execute_payment_spb($payment_id, $payer_id, $headers = array(), $bn_code_key = null)
	{
		$url = $this->get_base_url() . '/checkout/orders/' . $payment_id . '/capture';
	 ...
	 * @throws PayPal_Brasil_Connection_Exception
	 */
	public function update_payment($order_id, $data, $headers = array(), $bn_code_key = null)
	{
		$url = $this->get_base_url() . '/checkout/orders/' . $order_id;
		...
		// Get response.
		$response = $this->do_request('UPDATE_ORDER', $url, 'PATCH', $data, $headers);
		...
		throw new PayPal_Brasil_API_Exception(
			$code,
			__('Unable to update order.', "paypal-brasil-para-woocommerce")
		);
	}
	...
	{
		$url = $this->get_base_url_webhook() . '/notifications/verify-webhook-signature';
	...
	{
		$url = $this->get_base_url_webhook() . '/notifications/webhooks';
		...
		throw new PayPal_Brasil_API_Exception(
			$code,
			__('Unable to get the webhooks.', "paypal-brasil-para-woocommerce"),
			$response_body
		);
	}
	...
	public function create_webhook($webhook_url, $events)
	{
		...
		throw new PayPal_Brasil_API_Exception(
			$code,
			__('Could not create webhook.', "paypal-brasil-para-woocommerce"),
			$response_body
		);
	}
	 ...
	 * @throws PayPal_Brasil_Connection_Exception
	 */
	public function refund_payment($capture_id, $total = null, $currency = null)
	{
		$url = $this->get_base_url() . '/payments/captures/' . $capture_id . '/refund';
		...
		throw new PayPal_Brasil_API_Exception(
			$code,
			__('It was not possible to make a refund.', "paypal-brasil-para-woocommerce"),
			$response_body
		);
	}
