...
class PayPal_Brasil_API {
	...
	protected function do_request( $name, $url, $method = 'POST', $data = array(), $headers = array(), $log = true ) {
		...
		if ( isset( $params['body'] ) ) {
			WC_PAYPAL_LOGGER::log( __( "[{$name}] Making request ({$method}) for {$url}:\n" . $data . "\n" , "paypal-brasil-para-woocommerce" ), $this->gateway->id );
		} else {
			WC_PAYPAL_LOGGER::log( __( "[{$name}] Making request ({$method}) for {$url}\n", "paypal-brasil-para-woocommerce" ) , $this->gateway->id);
		}
		...
		if ( is_wp_error( $request ) ) {
			WC_PAYPAL_LOGGER::log( __( "[{$name}] HTTP error when making the request ({$method}) for {$url}\n", "paypal-brasil-para-woocommerce" ), $this->gateway->id );
		} else {
			// Only log response when $log exists.
			...
			if ( ! (preg_match( '/\/v1\/oauth2\/token$/', $url ) && $status_code >= 200 && $status_code <= 299 )) {
				WC_PAYPAL_LOGGER::log( __("[{$name}] Request response ({$method}) for {$url}:\n","paypal-brasil-para-woocommerce") . json_encode( $body,
						JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES ) . "\n", $this->gateway->id );
				WC_PAYPAL_LOGGER::log( __("[{$name}] Request response ({$method}) for {$url} complete:\n" . $raw_response . "\n","paypal-brasil-para-woocommerce"), $this->gateway->id );
			} else {
				WC_PAYPAL_LOGGER::log( __("[{$name}] Request response ({$method}) for {$url} with status code {$status_code} hidden for security reasons.\n", "paypal-brasil-para-woocommerce"), $this->gateway->id);
			}
		}
