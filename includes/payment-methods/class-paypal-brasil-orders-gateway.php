 ...
 */
class PayPal_Brasil_Plus_With_Orders_Gateway extends PayPal_Brasil_Gateway
{
	...
	public function __construct()
	{
		...
		$this->id = 'paypal-brasil-plus-with-orders-gateway';
		...
		$this->title = __('PayPal Brasil - Transparent Checkout orders v2', "paypal-brasil-para-woocommerce");
	...
	public function process_payment($order_id, $force = false)
	{
		...
		try {
			...
			if (empty($payer_id)) {
				wc_add_notice(__('An unexpected error occurred, please try again. If the error persists, please contact us. (#67)' . $response_data, "paypal-brasil-para-woocommerce"), 'error');
				// Set refresh totals to trigger update_checkout on frontend.
				WC()->session->set('refresh_totals', true);
				do_action('wc_ppp_brasil_process_payment_error', 'PAYER_ID', $order_id, null);
			...
			if (OrderUtil::custom_orders_table_usage_is_enabled()) {
				...
				$order->update_meta_data('wc_ppp_brasil_sale', $sale['purchase_units']);
				...
				update_post_meta($order_id, 'wc_ppp_brasil_sale', $sale['purchase_units']);
	...
	{
		$order_paypal_id = get_post_meta($order_id, 'wc_ppp_brasil_sale_id', true);
	...
	public function execute_payment($order, $order_id)
	{
		...
		$this->api->update_payment($order_id, $patch_data, array(), 'plus');
		$execution_response = $this->api->execute_payment($order_id, array(), 'plus');
	...
	public function get_posted_data()
	{
		...
		if (isset($payment['purchase_units'])) {
