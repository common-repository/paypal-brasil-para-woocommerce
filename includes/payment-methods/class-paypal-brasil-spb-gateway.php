...
class PayPal_Brasil_SPB_Gateway extends PayPal_Brasil_Gateway
{
	...
	public function get_title()
	{
		...
		if (is_admin()) {
			...
			return $pagenow === 'post.php' ? __(
				...
				"paypal-brasil-para-woocommerce"
			) : __('Digital Wallet', "paypal-brasil-para-woocommerce");
		}
	...
	public function init_form_fields()
	{
		$this->form_fields = array(
			...
			'invoice_id_prefix' => array(
				...
				'default' => '',
				'description' => __(
					'Add a prefix to the order number, this is useful for identifying you when you have more than one store processing through PayPal.',
					"paypal-brasil-para-woocommerce"
				),
			),
		);
	...
	private function validate_billing_agreement()
	{
		try {
			...
			$data = $ex->getData();
			if (
				isset($data['name']) && $data['name'] === 'AUTHORIZATION_ERROR'
	...
	private function is_processing_shortcut()
	{
		...
		// Check if is $_GET
		if (
			isset($_GET['review-payment']) && $_GET['review-payment']
		...
		if (isset($_POST['post_data'])) {
			...
			if (
				isset($post_data['paypal-brasil-shortcut-review-payment']) && $post_data['paypal-brasil-shortcut-review-payment']
		...
		// Check if is $_POST (checkout)
		if (
			isset($_POST['paypal-brasil-shortcut-review-payment']) && $_POST['paypal-brasil-shortcut-review-payment']
		...
		if (empty($post_data['paypal_brasil_billing_agreement'])) {
			...
			if (!empty($post_data['paypal_brasil_billing_agreement_token'])) {
				if (!$session_billing_agreement_token || $session_billing_agreement_token !== $post_data['paypal_brasil_billing_agreement_token']) {
					...
					wc_add_notice(
						__(
							'There was a problem verifying the payment settlement token session.',
							"paypal-brasil-para-woocommerce"
						),
						'error'
					);
					...
					try {
						...
						if (is_user_logged_in()) {
							update_user_meta(
								get_current_user_id(),
								'paypal_brasil_billing_agreement_id',
								$billing_agreement['id']
							);
							update_user_meta(
								get_current_user_id(),
								'paypal_brasil_billing_agreement_payer_info',
						...
						wc_add_notice(
							__(
								'There was an error creating the payment authorization.',
								"paypal-brasil-para-woocommerce"
							),
							'error'
						);
	...
	private function process_payment_shortcut($order)
	{
		...
		$data = array(
			array(
				...
				'value' => array(
					'value' => paypal_format_amount(wc_remove_number_precision_deep($order_total)),
					'currency_code' => $order->get_currency(),
					'breakdown' => array(
						'item_total' => array(
							'currency_code' => $order->get_currency(),
							'value' => paypal_format_amount(wc_remove_number_precision_deep($order_total - $order_shipping_total))
						),
						'shipping' => array(
							'currency_code' => $order->get_currency(),
							'value' => paypal_format_amount(wc_remove_number_precision_deep($order_shipping_total)),
						),
					),
				),
			),
		...
		if ($this->is_shortcut_override_address() && !paypal_brasil_is_order_only_digital($order)) {
			$data[] = array(
				...
				'value' => paypal_brasil_get_shipping_address_order_v2($order),
			);
		}
		...
		if (OrderUtil::custom_orders_table_usage_is_enabled()) {
			...
			$order->update_meta_data('paypal_brasil_sale_id', $response['purchase_units'][0]['payments']['captures'][0]['id']);
		} else {
			update_post_meta($order_id, 'paypal_brasil_execute_data', $response);
			update_post_meta($order_id, 'paypal_brasil_id', $response['id']);
			update_post_meta($order_id, 'paypal_brasil_sale_id', $response['purchase_units'][0]['payments']['captures'][0]['id']);
		}

		$order->add_order_note('Pagamento processado pelo PayPal. ID da transação:' . $response['purchase_units'][0]['payments']['captures'][0]['id']);
		$order->save();
		// Process the order.
		...
		if (!is_user_logged_in()) {
			try {
				...
				if (!$user) {
					...
					$user_username = wc_create_new_customer_username(
						$user_email,
						array(
	...
	{
		$installment = isset($_POST['paypal_brasil_billing_agreement_installment']) ? json_decode(
			stripslashes($_POST['paypal_brasil_billing_agreement_installment']),
			true
		) : array();
		...
		if (!$uuid) {
			wc_add_notice(
				__(
					'There was a problem verifying the fraudnet token.',
					"paypal-brasil-para-woocommerce"
				),
				'error'
			);
		...
		$data = array(
			'purchase_units' => array(
				array(
					'items' => array(
						'name' => sprintf(
							__('Store order %s', "paypal-brasil-para-woocommerce"),
							get_bloginfo('name')
						),
						'unit_amount' => array(
		...
		if (!$only_digital_items) {
			$shipping = paypal_brasil_get_shipping_address_order_v2($order);
		...
		if ($discount_value = floatval($installment['discount_amount']['value'])) {
			...
			$discount->set_name(
				sprintf(
					__('PayPal discount (%d%%)', "paypal-brasil-para-woocommerce"),
		...
		$order->add_order_note('Payment processed by PayPal. Transaction ID:' . $response['purchase_units'][0]['payments']['captures'][0]['id']);
		$order->save();
		return array(
			'result' => 'success',
			'redirect' => $this->get_return_url($order),
	...
	private function process_payment_spb($order)
	{
		...
		if (OrderUtil::custom_orders_table_usage_is_enabled()) {
			...
			$order->add_order_note('Adicionou via wordpress');
			update_post_meta($order_id, 'paypal_brasil_execute_data', $response);
			update_post_meta($order_id, 'paypal_brasil_id', $response['id']);
			update_post_meta(
				$order_id,
		...
		try {
			...
			switch ($data['name']) {
				...
				case 'INTERNAL_SERVICE_ERROR':
					wc_add_notice(
						__(
							'An unexpected error occurred, please try again. If the error persists, please contact us. (#01)',
							"paypal-brasil-para-woocommerce"
						),
						'error'
					);
				...
				case 'VALIDATION_ERROR':
					wc_add_notice(
						__(
							'An unexpected error occurred, please try again. If the error persists, please contact us. (#12)',
							"paypal-brasil-para-woocommerce"
						),
						'error'
					);
				...
				case 'PAYMENT_ALREADY_DONE':
					wc_add_notice(
						__('A payment already exists for this order.', "paypal-brasil-para-woocommerce"),
						'error'
					);
					break;
				default:
	...
	public function process_refund($order_id, $amount = null, $reason = '')
	{
		if (OrderUtil::custom_orders_table_usage_is_enabled()) {
			...
			if (!empty($orders)) {
				$sale_id = $orders[0]->get_meta('paypal_brasil_sale_id');
			} else {
				$sale_id = '';
			}
		} else {
		...
		if ($amount <= 0) {
			...
			return new WP_Error(
				'error',
				sprintf(
					__('The refund cannot be less than %s.', "paypal-brasil-para-woocommerce"),
		...
		if ($sale_id) {
			try {
				$refund_sale = $this->api->refund_payment(
					$sale_id,
					paypal_brasil_money_format($amount),
			...
			return new WP_Error('error', sprintf(
				__(
					'It looks like you don\'t have a request for a refund.',
					"paypal-brasil-para-woocommerce"
				)
			) . "aqui" . $order_id);
		}
	...
	public function admin_scripts()
	{
		...
		if ($wc_settings_id === $screen_id && isset($_GET['section']) && $_GET['section'] === $this->id) {
			...
			wp_enqueue_script(
				$this->id . '_script',
				plugins_url('assets/dist/js/admin-options-spb.js', PAYPAL_PAYMENTS_MAIN_FILE),
				array(),
				PAYPAL_PAYMENTS_VERSION,
				true
			);
			wp_localize_script(
				$this->id . '_script',
				'paypal_brasil_admin_options_spb',
				array(
			...
			wp_enqueue_style(
				$this->id . '_style',
				plugins_url('assets/dist/css/admin-options-spb.css', PAYPAL_PAYMENTS_MAIN_FILE),
				array(),
				PAYPAL_PAYMENTS_VERSION,
				'all'
			);
	...
	public function checkout_scripts()
	{
		...
		if (is_checkout()) {
			if ($this->is_reference_transaction()) { // reference transaction checkout
				...
				ob_start();
				wc_print_notice(
					__('You canceled the creation of the payment term.', "paypal-brasil-para-woocommerce"),
					'error'
				);
		...
		// Shortcut
		if ($this->enabled === 'yes' && $this->is_shortcut_enabled() && !is_checkout()) {
			$enqueues[] = array(
			...
			wp_enqueue_style(
				'paypal-brasil-shortcut',
				plugins_url('assets/dist/css/frontend-shortcut.css', PAYPAL_PAYMENTS_MAIN_FILE),
				array(),
				PAYPAL_PAYMENTS_VERSION,
				'all'
			);
