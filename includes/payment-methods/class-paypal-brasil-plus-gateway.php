...
class PayPal_Brasil_Plus_Gateway extends PayPal_Brasil_Gateway
{
	...
	public function process_payment($order_id, $force = false)
	{
		...
		try {
			...
			if (empty($response_data)) {
				WC_PAYPAL_LOGGER::log("The iframe could not be intercepted to process payment.\n",$this->id);
	...
	public function process_refund($order_id, $amount = null, $reason = '')
	{
		if (OrderUtil::custom_orders_table_usage_is_enabled()) {
			...
			if (!empty($orders)) {
				$sale_id = $orders[0]->get_meta('wc_ppp_brasil_sale_id');
			} else {
				$sale_id = '';
			}
		} else {
		...
		WC_PAYPAL_LOGGER::log("Sale id:" . $sale_id, $this->id);
	...
	public function checkout_scripts()
	{
		...
		if (is_checkout() && !get_query_var('order-received')) {
			...
			wp_enqueue_script('ppp-script', 'https://www.paypalobjects.com/webstatic/ppplusdcc/ppplusdcc.min.js', array(), PAYPAL_PAYMENTS_VERSION, true);
	...
	public function get_title()
	{
		...
		if (is_admin()) {
			...
			return $pagenow === 'post.php' ? __('PayPal - Transparent Checkout (PPP)', "paypal-brasil-para-woocommerce") : __('Transparent checkout (PPP)', "paypal-brasil-para-woocommerce");
		}
